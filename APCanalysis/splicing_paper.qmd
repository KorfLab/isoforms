...
if you have 100,000 counts in a gene, make 100,000 counts in the other one
then do stats
Kind of instead of dropping 0s
Cutoff of APC output would change depending on how many counts are in the APC
Prob wouldn't get anything new
Just drop the 0s

Just chebyshev, manhattan not useful anymore
What if there are 2 bad introns? cheby would not tell us which those are
Cheby has a list of difs, so the information is there

ratio is more interesting than absolute dif? instead of cheby
greatest ratio difference
