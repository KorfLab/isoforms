...
for item in gene_lines_2.items():
	...
	for line in item[1]:
		if line[2] == 'CDS':
			...
			if 1 <= n_cds < (total_cds - 1):
				total_cds_len += int(line[4]) - int(line[3]) + 1
				gene_lines_3[item[0]].append(new_line)
				n_cds += 1
				continue
				
			# adjust last CDS coor and add length
			else:
				last_cds_len = int(item[0][3]) - int(line[3]) + 1
				total_cds_len += last_cds_len
