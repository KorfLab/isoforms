...
```{r}
wsdpwm <- read.csv('weighted_smallgenes_donor_pwm.csv', header = FALSE)
colnames(wsdpwm) <- c('A', 'C', 'G', 'T')
```
```{r}
wsapwm <- read.csv('weighted_smallgenes_acceptor_pwm.csv', header = FALSE)
colnames(wsapwm) <- c('A', 'C', 'G', 'T')
```
```{r}
sdpwm <- read.csv('smallgenes_donor_pwm.csv', header = FALSE)
colnames(sdpwm) <- c('A', 'C', 'G', 'T')
```
```{r}
sapwm <- read.csv('smallgenes_acceptor_pwm.csv', header = FALSE)
colnames(sapwm) <- c('A', 'C', 'G', 'T')
```

```{python}
wsdpwm_plot = logomaker.Logo(r.wsdpwm, shade_below=.5,
fade_below=.5)
