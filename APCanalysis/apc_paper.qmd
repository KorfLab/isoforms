...
editor: visual
---
Before installing, run these in terminal:
sudo apt install make
sudo apt install gcc
sudo apt install g++
reticulate also needs:
...
```{python}
wsapwm_plot = logomaker.Logo(r.wsapwm, shade_below=.5,
fade_below=.5)
long_dpwm.ax.set_ylim(0,2)
plt.show()
```
...
```{python}
sdpwm_plot = logomaker.Logo(r.sdpwm, shade_below=.5,
fade_below=.5)
long_dpwm.ax.set_ylim(0,2)
plt.show()
```
...
```{python}
sapwm = logomaker.Logo(r.sapwm, shade_below=.5,
fade_below=.5)
long_dpwm.ax.set_ylim(0,2)
plt.show()
```
