...
GO analysis WormBase has GO built in? not sure how to use
libcurl need for httr and others
...
Need to check if genes are up or down regulated after association.
Can be down with thousands of pathways, need to account for multiple testing. Most commonly used Benjamini-Hochberg correction.
...
Sanbomics
BgRatio is background ratio, so 9,828 genes in the worm database
But there are around 20k genes in the worm genome?
```{r}
...
Compare withing RNA-seq intron distance with APC output?
There is about a 30% difference between introns...should that be considered when comparing APC to RNAseq? yes but it doesn't change distance
...
Gene set enrichment is different from GO enrichment?
GSE needs log fold change, we do not have that


Read through the entire clusterprofiler vignette?
