...
Before installing, run these in terminal: "sudo apt install make", "sudo apt install gcc", "sudo apt install g++" reticulate also needs: "sudo apt-get install -y libpng-dev"
...
Make sure these packages are installed in the conda environment

Import each one individually or else Rstudio will crash
```{python}
import numpy as np
import pandas as pd
...
## Smallgenes
There is a sharp exponential increase in the number of possible isoforms that can be generated from a sequence of increasing length (Figure 1). It is impossible to test genes above a length of around 1000 base pairs before the compute time is unmanageable. Sequences of length 1200 bp have the potential to take sround 80 hours of compute time (Figure 2) Because of this, we have created a subset of the C. elegans genome called the smallgenes dataset. Each gene is represented by a single fasta file and GFF3 annotion file. This dataset has the following restrictions on each gene: no overlaps with other genes or genomic features, the gene is flanked... this can just be copy pasted from KorfLab/smallgenes
...
But does having each site count differently than mean each site is on it's own scale and can't be compared?

```{r}
pwms <- read.csv('test.pwm', header = FALSE)
split_pwms <- split(pwms, pwms$V1) 

wbdpwm <- split_pwms$smallgenes_wormbase_donor_pwm %>% 
  select(, -1) 
colnames(wbdpwm) <- c('A', 'C', 'G', 'T')

wbapwm <- split_pwms$smallgenes_wormbase_acceptor_pwm   %>% 
  select(, -1)
colnames(wbapwm) <- c('A', 'C', 'G', 'T')

rsdpwm <- split_pwms$smallgenes_rnaseq_donor_pwm %>% 
  select(, -1)
colnames(rsdpwm) <- c('A', 'C', 'G', 'T')

rsapwm <- split_pwms$smallgenes_rnaseq_acceptor_pwm %>% 
  select(, -1)
colnames(rsapwm) <- c('A', 'C', 'G', 'T')
```

```{python}
wbdpwm_plt = logomaker.Logo(r.wbdpwm, shade_below=.5,
fade_below=.5)
...
```{python}
wbapwm_plot = logomaker.Logo(r.wbapwm, shade_below=.5,
fade_below=.5)
long_dpwm.ax.set_ylim(0,2)
plt.show()
```
...
```{python}
rsdpwm_plot = logomaker.Logo(r.rsdpwm, shade_below=.5,
fade_below=.5)
long_dpwm.ax.set_ylim(0,2)
plt.show()
```
...
```{python}
rsapwm = logomaker.Logo(r.rsapwm, shade_below=.5,
fade_below=.5)
long_dpwm.ax.set_ylim(0,2)
plt.show()
```
...
Interpreting results of the APC algorithm

```{r}
dist_bins <- read.csv('bins.csv', header = FALSE)
ggplot(dist_bins, aes(x=V1, y=V2)) +
  geom_bar(stat = "identity")
```

Take one really bad gene, why is it broken?
Nmd filter?